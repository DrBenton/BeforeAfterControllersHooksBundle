Feature:
    ...
    Scenario: Run multiple Controller class self-contained methods before running its target Action, with a params transmission
        Given I have a Controller with a @Before("beforeAction", args={"Good morning", "Mr. Phelps"}) Action Annotation
        And a @Before("beforeAction", args={"Well", "nobody’s perfect!"}) Action Annotation
        And a self-contained method in a Controller with the following content:
            """
